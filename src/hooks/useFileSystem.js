...
const dedent = (str) => {
    const base = str.startsWith('\n') ? str.slice(1) : str;
    const minIndent = base.split('\n').filter(line => line.trim()).reduce((min, line) => {
        const indent = line.match(/^\s*/)[0].length;
        return Math.min(min, indent);
    }, Infinity);
    if (minIndent === Infinity) {
...
const updateItemInTree = (items, path, updater) => {
    return items.map(item => {
        ...
        if (item.type === 'folder' && item.children) {
            return { ...item, children: updateItemInTree(item.children, path, updater) };
        }
        return item;
    });
};
...
const addToFolder = (items, folderPath, newItem) => {
    return items.map(item => {
        ...
        if (item.type === 'folder' && item.children) {
            return { ...item, children: addToFolder(item.children, folderPath, newItem) };
        }
        return item;
    });
};
...
// These functions are now empty or return null to disable persistence.
const saveFileTree = (tree) => {
    // localStorage.setItem('shonax_file_tree', JSON.stringify(tree));
};
...
export const useFileSystem = () => {
    ...
    useEffect(() => {
        ...
        // Since loading is disabled, we always start with the default files.
        const treeToUse = DEFAULT_FILES;
        setFileTree(treeToUse);
        ...
        const allFiles = flattenFileTree(treeToUse);
        const firstFile = allFiles.find(f => f.name === '01_variables.shona') || allFiles[0];
        if (firstFile) {
            setCurrentFile(firstFile);
            setCode(firstFile.content || '');
            setOpenTabs([firstFile]);
    ...
    const handleFileSelect = useCallback((file) => {
        if (!file || file.type !== 'file' || file.path === currentFile?.path) return;
    ...
    const updateCurrentCode = useCallback((newCode) => {
        ...
        if (currentFile && newCode !== currentFile.content) {
            const markAsModified = (f) => ({ ...f, modified: true });
            setCurrentFile(prev => markAsModified(prev));
            setOpenTabs(prev => prev.map(t => (t.path === currentFile.path ? markAsModified(t) : t)));
            setFileTree(prev => updateItemInTree(prev, currentFile.path, f => ({...f, content: newCode, modified: true})));
        }
    }, [currentFile]);

    const handleSaveFile = useCallback(() => {
        if (!currentFile || !currentFile.modified) return;
        console.log("Saving is disabled. Visually marking as saved.");
        
        const markAsSaved = (f) => ({ ...f, content: code, modified: false });
        setCurrentFile(prev => markAsSaved(prev));
        setOpenTabs(prev => prev.map(t => (t.path === currentFile.path ? markAsSaved(t) : t)));
        setFileTree(prev => updateItemInTree(prev, currentFile.path, f => ({...f, content: code, modified: false})));
    }, [currentFile, code]);

    const handleCloseTab = useCallback((tab) => {
        // No need to confirm save since persistence is disabled
        const newTabs = openTabs.filter(t => t.path !== tab.path);
        setOpenTabs(newTabs);

        if (currentFile?.path === tab.path) {
    ...
    }, [openTabs, currentFile, handleFileSelect]);

    const handleFileCreate = useCallback((parentPath, fileName) => {
        alert("Creating new files is disabled in this playground mode.");
    }, []);
    
    const handleFolderCreate = useCallback((parentPath, folderName) => {
        alert("Creating new folders is disabled in this playground mode.");
    }, []);
    
    const handleDelete = useCallback((path) => {
        alert("Deleting files/folders is disabled in this playground mode.");
    }, []);
    
    const handleRename = useCallback((oldPath, newName) => {
        alert("Renaming files/folders is disabled in this playground mode.");
    }, []);

    const getAllFiles = useCallback(() => flattenFileTree(fileTree), [fileTree]);

    return {
        fileTree, currentFile, openTabs, code, setCode: updateCurrentCode, filesInitialized, expandedFolders,
